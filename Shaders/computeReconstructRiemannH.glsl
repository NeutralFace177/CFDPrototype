...
void main() {
    ...
    float uR = Scheme(1,1,false) + rhieChowToggle * RCYF;
    float uL = Scheme(1,1,true) + rhieChowToggle * RCYB;
    ...
    float vhat = (sqrt(dL)*vL+sqrt(dR)*uR)/dLdR;
    float S_L = vhat - dhat;
    float S_R = vhat + dhat;
    ...
    if (0 <= S_L) {
        ...
        float EML = dSvSSL * (EL/dL + (S_M-vL)*(S_M+pL/(dL*(S_L-uL))));
        float sML = dSvSSL * sL;
        ...
        float EMR = dSvSSR * (ER/dR + (S_M-vR)*(S_M+pR/(dR*(S_R-uR))));
        float sMR = dSvSSR * sR;
        ...
        yFaceFlux[faceIndex].E = sR + S_L*(sMR-sR);
    } else if (0 >= S_R) {
