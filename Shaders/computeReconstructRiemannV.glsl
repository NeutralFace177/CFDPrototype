...
layout (std430, binding = 8) buffer out_fluxes {
...
float BC(int valId, int I, int J, int iOffset, int jOffset) {
    uint newIndex = cellCoordToIndex(int(clamp(I+iOffset,0,int(gl_NumWorkGroups.x-2))),int(clamp(J+jOffset,0,int(gl_NumWorkGroups.y-1))));
    ...
    if (objectFlag) {
        ...
        switch (valId) {
            ...
            //e
            case 3:
                return fields[newIndex].E - 0.5 * (fields[newIndex].u * fields[newIndex].u + fields[newIndex].v * fields[newIndex].v) + 0.5;
            //S 
            case 4:
                return fields[newIndex].S;
        }
...
void main() {
    ...
    float pr = calcPressure(1,0);
    float pl = calcPressure(-1,0);

    //U_K
    float pR = (pc + pr)/2.0;
    float pL = (pc + pl)/2.0;

    float dR = Scheme(0,0,false);
    float dL = Scheme(0,0,true);
    ...
    float sR = Scheme(4,0,false);
    float sL = Scheme(4,0,true);
