...
float BC(int valId, int iOffset, int jOffset) {
    ...
    if (objectFlag) {
        ...
        switch (valId) {
            ...
            //e
            case 3:
                return fields[newIndex].E - 0.5 * (fields[newIndex].u*fields[newIndex].u + fields[newIndex].v*fields[newIndex].v) + 0.5*1;
            //S 
            case 4:
                return fields[newIndex].S;
        }
...
//        outFields[index].E = (4.0*fields[index].E-prevFields[index].E + (1.0 / fields[index].d) * dt * (-(uXF * (dXF * EXF + pressureToggle * pXFC) - uXB * (dXB * EXB + pressureToggle * pXBC)) / dx
  //                      - (vYF * (dYF * EYF + pressureToggle * pYFC) - vYB * (dYB * EYB + pressureToggle * pYBC)) / dy
    ...
    vec3 sEdVIEW = vec3(sqrt(outFields[index].u*outFields[index].u+outFields[index].v*outFields[index].v)/1.0,outFields[index].E / 300.0,outFields[index].d/2.5);
